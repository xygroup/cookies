...
Cookies.prototype = {
  get: function(name, opts) {
    var sigName = name + ".sig"
      ...
      , update = opts && opts.update !== undefined ? opts.update : !!this.response
    ...
    // Updates require access to a response, which we don't have in a WebSocket context.
    if (update && index < 0) {
      this.set(sigName, null, {path: "/", signed: false })
    } else {
      update && index && this.set(sigName, this.keys.sign(data), { signed: false })
      return value
    }
  },
